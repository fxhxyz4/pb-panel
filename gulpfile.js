const path = {
  ...
  sprite: {
    i: [`build/img/svg/*.svg`, `build/img/*.svg`],
    o: `build/img/svg/`,
  },
...
const watching = () => {
  watch(path.watching.i[0], styles).on(
    `change`,
    browserSync.reload
  );

  watch(path.watching.i[1], scripts).on(
    `change`,
    browserSync.reload
  );

  watch(path.watching.i[2], html).on(
    `change`,
    browserSync.reload
  );

  watch(path.watching.i[3], images).on(
    `change`,
    browserSync.reload
  );
};
...
const buildDist = () => {
  return src(path.build.i, { base: path.src.i }).pipe(
    dest(path.build.o)
  );
};
